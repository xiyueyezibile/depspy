...
class ModuleGraph {
  ...
  transform(entryId: string = this.entryId ,depth: number = 9999, parent: ModuleTree | null = null): ModuleTree | null {
    ...
    const tree: ModuleTree = {
        ...
        name: nameArr.slice(nameArr.length >= 2? nameArr.length - 2: 0).join(path.sep),
        children: [],
...
class Bundle {
  ...
  constructor(options: Config) {
    ...
    if(options.pkgPath) {
        ...
        this.externals = [...(Object.keys(pkgJson.dependencies || [])), ...(Object.keys(pkgJson.devDependencies || []))]
    }
  ...
  /** 获取实际被打包的模块 */
  resolveOriginModuleByBundle(fn: (originModules: Map<string, any>, originExternalModules: Map<string, any>, externals: string[]) => void) {
  ...
  resolveLoadModule(id: string) {
    ...
    if (!idInExternals(id)) {
      ...
      this.loadModules.set(id, id)
        ...
        const externalName = externalArr.length === 3? externalArr[0] + '@' + externalArr[1]: externalArr[0]
        if(this.externals.includes(externalName)) this.externalModules.set(getExternalName(id), getExternalName(id))
    }
  }
  /** 寻找在编译模块和实际模块都存在的模块 */
...
export function vitePluginInsight(options: Config): any{
  ...
  return {
    ...
    generateBundle(_, bundle) {
      ...
      // 根据bundle获取实际被打包的模块
      globleBundle.resolveOriginModuleByBundle((originModules, originExternalModules, externals) => {
          ...
          Object.entries(dist["modules"] || {}).forEach(([id, data]) => {
            if(!idInExternals(id)) {
              originModules.set(id, data)
                ...
                const externalArr = getExternalName(id).split('@')
                const externalName = externalArr.length === 3? externalArr[0] + '@' + externalArr[1]: externalArr[0]
                if(externals.includes(externalName)) originExternalModules.set(getExternalName(id), [...(originExternalModules.get(getExternalName(id)) || []), data])
            }
